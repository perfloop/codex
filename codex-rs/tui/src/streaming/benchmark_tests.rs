...
fn growing_paragraph_chunks(chunk_count: usize) -> Vec<String> {
    (0..chunk_count)
        .map(|index| {
            format!(
                "Paragraph line {index:04} stays in the same mutable markdown block for streaming.\n"
            )
        })
        .collect()
}
...
fn unclosed_fence_chunks(chunk_count: usize) -> Vec<String> {
    ...
    chunks.extend((1..chunk_count).map(|index| {
        format!(
            "let streamed_value_{index:04} = {index}; // fence remains unclosed while streaming\n"
        )
    }));
    chunks
}
...
fn growing_table_chunks(chunk_count: usize) -> Vec<String> {
    ...
    chunks.extend((2..chunk_count).map(|index| {
        format!("| {index:04} | This table row keeps the table tail mutable while streaming. |\n")
    }));
    chunks
}
...
fn perfloop_streaming_rerender_sample() {
    ...
    black_box(run_workload(&fixtures, &cwd));
