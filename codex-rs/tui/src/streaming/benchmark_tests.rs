...
fn expected_lines(source: &str) -> Vec<String> {
    expected_lines_at_width(source, RENDER_WIDTH)
}

fn expected_lines_at_width(source: &str, width: usize) -> Vec<String> {
    let mut expected = Vec::new();
    append_markdown_agent(source, Some(width), &mut expected);
    plain_lines(&expected)
}

fn verify_fixture_at_presentation_boundaries(
    fixture: &Fixture,
    cwd: &Path,
    presentation_chunks: usize,
) {
    ...
    for (index, chunk) in fixture.chunks.iter().enumerate() {
        ...
        assert_eq!(
            ...
            "{} stream diverged at {}-chunk presentation boundary after {} chunks",
            fixture.kind,
            presentation_chunks,
            index + 1,
        );
...
fn verify_resize_preserves_table_tail(cwd: &Path) {
    let mut controller = StreamController::new(Some(120), cwd, HistoryRenderMode::Rich);
    let source = "| Key | Description |\n| --- | --- |\n| one | value that wraps after resize |\n";
    ...
    let (cell, finalized_source) = controller.finalize();
    assert_eq!(finalized_source.as_deref(), Some(source));
    let actual = cell
        .map(|cell| strip_agent_prefix(cell.transcript_lines(u16::MAX)))
        .unwrap_or_default();
    assert_eq!(plain_lines(&actual), expected_lines_at_width(source, 32));
}

fn verify_source_wide_fallbacks(cwd: &Path) {
    let mut source = String::new();
    ...
    for chunk in [
        ...
        assert_eq!(
            ...
            "reference definition fallback diverged after {chunk:?}",
        );
    }
    ...
    for chunk in [
        ...
        assert_eq!(
            ...
            "inline visualization fallback diverged after {chunk:?}",
        );
    }
}
...
fn presentation_boundaries_preserve_output() {
    ...
    for presentation_chunks in [1, DEFAULT_PRESENTATION_CHUNKS] {
        for fixture in fixtures.iter().filter(|fixture| fixture.chunk_count <= 100) {
            verify_fixture_at_presentation_boundaries(fixture, &cwd, presentation_chunks);
        }
    }
    verify_resize_preserves_table_tail(&cwd);
    verify_source_wide_fallbacks(&cwd);
}
