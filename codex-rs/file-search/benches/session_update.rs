...
fn take_metadata_probe_count(counter_file: &Path) -> usize {
    let bytes = fs::read(counter_file).expect("read metadata probe counter");
    let count = u64::from_ne_bytes(
        bytes
            .as_slice()
            .try_into()
            .expect("metadata probe counter is one u64"),
    );
    usize::try_from(count).expect("metadata probe count fits usize")
}
